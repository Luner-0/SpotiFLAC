...
// before it has been downloaded. Audio is resampled to mono 44.1kHz and capped in
// length to keep the transfer reasonable.
func DecodeAudioForAnalysisURL(url string) (*AnalysisDecodeResponse, error) {
	if url == "" {
		return nil, fmt.Errorf("url is required")
	}
	pcmBase64, err := extractAnalysisPCMBase64URL(url, 120)
	if err != nil {
		return nil, err
	}
